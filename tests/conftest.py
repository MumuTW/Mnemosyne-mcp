...
from mnemosyne.interfaces.graph_store import GraphStoreClient, QueryResult, ConnectionConfig
from mnemosyne.api.main import app, get_graph_client, get_current_settings
...
class MockGraphStoreClient(GraphStoreClient):
    ...
    async def execute_query(self, query: str, parameters=None, trace_id=None) -> QueryResult:
        """模擬查詢執行"""
        # 模擬簡單的測試查詢
...
def test_settings() -> Settings:
    """測試配置"""
    from mnemosyne.core.config import DatabaseSettings, APISettings, LoggingSettings
...
async def mock_graph_client(test_settings) -> AsyncGenerator[MockGraphStoreClient, None]:
    """模擬圖資料庫客戶端 fixture"""
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        # 為所有測試添加適當的標記
        if "unit" not in item.keywords and "integration" not in item.keywords and "e2e" not in item.keywords:
