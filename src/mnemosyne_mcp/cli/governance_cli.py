    ...
    def _format_human(self, result: ValidationResult) -> str:
        ...
        if result.violations:
            for violation in result.violations:
                ...
                lines.append(
                    f"{severity_emoji} {violation.location.file_path}:{violation.location.line_number}"
                )
...
class GovernanceCLI:
    ...
    async def pr_check(
        ...
        try:
            ...
            final_result = ValidationResult(
                success=True, total_files_checked=total_files
            )
            ...
            if severity_threshold == "error" and final_result.has_errors():
                ...
                final_result.has_errors() or final_result.warning_count > 0
            ):
                return 1
    ...
    async def _parse_file_ast(self, file_path: str) -> Dict:
        ...
        if "ui" in file_path:
            return {
                "imports": [{"module": "src.data.user_repository", "line_number": 3}],
                "functions": [],
                "function_calls": [],
            }
        elif "service" in file_path:
            return {
                "imports": [{"module": "src.data.user_repository", "line_number": 2}],
                "functions": [{"name": "get_user", "start_line": 10, "end_line": 20}],
                "function_calls": [{"name": "eval", "line_number": 15}],
            }
        else:
            return {"imports": [], "functions": [], "function_calls": []}
...
def pr_check_command(
    target_branch: str, config_path: str, format_type: str, severity_threshold: str
):
    """檢查 PR 中的程式碼約束違規"""
    cli = GovernanceCLI()
