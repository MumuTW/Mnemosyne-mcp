...
class LoggingSettings(BaseSettings):
    ...
    # 處理器配置
    handlers: List[Dict[str, Any]] = Field(default_factory=lambda: [
...
def load_yaml_config_if_exists() -> Dict[str, Any]:
    ...
    if not config_file.exists():
        print(f"Info: Config file {config_file} not found, using defaults and environment variables")
        return {}
...
def validate_config(settings: Settings) -> List[str]:
    ...
    # 驗證安全配置
    if settings.is_production and settings.security.secret_key == "dev-secret-key-change-in-production":
