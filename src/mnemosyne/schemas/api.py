...
class HealthResponse(BaseModel):
    ...
    # 組件健康狀態
    components: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="組件狀態")
...
class IngestResponse(BaseModel):
    ...
    estimated_duration_minutes: Optional[int] = Field(default=None, description="預估耗時（分鐘）")
...
class SearchResult(BaseModel):
    ...
    # 關係信息
    relationships: List[Dict[str, Any]] = Field(default_factory=list, description="相關關係")
...
class ImpactAnalysisResponse(BaseModel):
    ...
    # 詳細影響
    direct_impacts: List[Dict[str, Any]] = Field(default_factory=list, description="直接影響")
    indirect_impacts: List[Dict[str, Any]] = Field(default_factory=list, description="間接影響")
