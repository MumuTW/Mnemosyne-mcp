...
class CallsRelationship(BaseRelationship):
    ...
    表示一個函數調用另一個函數的關係。
    """
    relationship_type: RelationshipType = Field(default=RelationshipType.CALLS, frozen=True)
    
    # 調用特性
    call_type: str = Field(default="direct", description="調用類型：direct, indirect, recursive")
...
class ContainsRelationship(BaseRelationship):
    ...
    """
    relationship_type: RelationshipType = Field(default=RelationshipType.CONTAINS, frozen=True)
    ...
    # 可見性
    visibility: str = Field(default="public", description="可見性：public, private, protected")
...
class DependsOnRelationship(BaseRelationship):
    ...
    表示一個實體依賴另一個實體的關係。
    """
    relationship_type: RelationshipType = Field(default=RelationshipType.DEPENDS_ON, frozen=True)
...
class InheritsFromRelationship(BaseRelationship):
    ...
    表示類繼承關係。
    """
    relationship_type: RelationshipType = Field(default=RelationshipType.INHERITS_FROM, frozen=True)
...
class ImplementsRelationship(BaseRelationship):
    ...
    表示類實現介面的關係。
    """
    relationship_type: RelationshipType = Field(default=RelationshipType.IMPLEMENTS, frozen=True)
...
class ImportsRelationship(BaseRelationship):
    ...
    表示模組導入關係。
    """
    relationship_type: RelationshipType = Field(default=RelationshipType.IMPORTS, frozen=True)
...
class UsesRelationship(BaseRelationship):
    ...
    表示一個實體使用另一個實體的關係。
    """
    relationship_type: RelationshipType = Field(default=RelationshipType.USES, frozen=True)
...
class AppliesToRelationship(BaseRelationship):
    ...
    表示約束或規則應用到實體的關係。
    """
    relationship_type: RelationshipType = Field(default=RelationshipType.APPLIES_TO, frozen=True)
    
    # 應用特性
    constraint_type: str = Field(description="約束類型")
    severity: str = Field(default="medium", description="嚴重程度：low, medium, high, critical")
